...
def get_ideal_weight_gain_loss_factor(user: User):
    ...
    ''', (user.id,))
    weight_data = [(item[0], datetime.strptime(item[1], '%Y-%m-%d %H:%M:%S')) for item in cursor.fetchall()]
    ...
    daily_calories_needs = nu_utils.calculate_daily_caloric_needs(user.weight, user.height, user.age, user.get_user_activity_level())
...
def update_weight_gain_loss_factor(user: User):
    factors = get_ideal_weight_gain_loss_factor(user)
    # implement algorithm: ------
